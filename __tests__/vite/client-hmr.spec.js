...
const applyClientHMR = require('../../lib/vite/client-hmr');

function whenHotTriggeredWith(changedFiles) {
  const listenerCallback = mockImport.meta.hot.on.mock.calls[0][1];
  return listenerCallback({ changedFiles });
}

describe('Vite - client-hmr', () => {
  let i18nMock;
  let reloadError;
  ...
  beforeEach(() => {
    ...
    mockImport.meta.hot.on.mockReset();
  });
  ...
  it('should register a listener on the key `i18next-hmr:locale-changed`', () => {
    applyClientHMR(i18nMock);
    expect(mockImport.meta.hot.on).toHaveBeenCalledWith(
      'i18next-hmr:locale-changed',
      expect.any(Function)
    );
  });
